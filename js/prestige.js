...
PrestigeUpgrade.init = function() {
	...
	for (var i = 0; i < prestigeUpgrades.length; i++) {
		...
		$("#prestige-upgrades").append('<li id="prestige-upgrade-' + (i+1) + '" class="list-group-item cur-p"><b>' + cnItem(p.name) + '</b> : 成本 ' + fix(p.price, "money") + ' 经验');
	...
	for (var i = 0; i < prestigeShoot.length; i++) {
		...
		$("#prestige-shoot").append('<li id="prestige-shoot-' + (i+1) + '" class="list-group-item cur-p"><b>' + cnItem(p.name) + '</b> : 成本 ' + fix(p.price, "money") + ' 经验');
	...
	for (var i = 0; i < prestigeShooting.length; i++) {
		...
		$("#prestige-shooting").append('<li id="prestige-shooting-' + (i+1) + '" class="list-group-item cur-p"><b>' + cnItem(p.name) + '</b> : 成本 ' + fix(p.price, "money") + ' 经验');
	...
	for (var i = 0; i < prestigeReloading.length; i++) {
		...
		$("#prestige-reloading").append('<li id="prestige-reloading-' + (i+1) + '" class="list-group-item cur-p"><b>' + cnItem(p.name) + '</b> : 成本 ' + fix(p.price, "money") + ' 经验');
...
PrestigeUpgrade.saveCheck = function() {
	...
	for (var i = 0; i < prestigeUpgrades.length; i++) {
		...
		if (prestigeUpgradesOwned[i] == true) {
			...
			$("#prestige-upgrade-" + (i+1)).append('<span class="badge">已拥有</span>');
		};
	};
	for (var i = 0; i < prestigeShoot.length; i++) {
		var p = prestigeShoot[i];
		if (prestigeShootOwned[i] == true) {
			...
			$("#prestige-shoot-" + (i+1)).append('<span class="badge">已拥有</span>');
		};
	};
	for (var i = 0; i < prestigeShooting.length; i++) {
		var p = prestigeShooting[i];
		if (prestigeShootingOwned == true) {
			...
			$("#prestige-shooting-" + (i+1)).append('<span class="badge">已拥有</span>');
	...
	for (var i = 0; i < prestigeReloading.length; i++) {
		...
		if (prestigeReloadingOwned == true) {
			...
			$("#prestige-reloading-" + (i+1)).append('<span class="badge">已拥有</span>');
...
PrestigeUpgrade.buy = function(type, index) {
	if (type == "drugs" && prestige[0] >= prestigeUpgrades[index].price) {
		...
		$("#prestige-upgrade-" + (index+1)).append('<span class="badge">已拥有</span>');
	};
	if (type == "shoot" && prestige[0] >= prestigeShoot[index].price) {
		...
		$("#prestige-shoot-" + (index+1)).append('<span class="badge">已拥有</span>');
	};
	if (type == "autoshoot" && prestige[0] >= prestigeShooting[index].price) {
		...
		$("#prestige-shooting-" + (index+1)).append('<span class="badge">已拥有</span>');
		$("#actions-auto-shoot").css("display", "block");
	};
	if (type == "autoreload" && prestige[0] >= prestigeReloading[index].price) {
		...
		$("#prestige-reloading-" + (index+1)).append('<span class="badge">已拥有</span>');
		$("#actions-auto-reload").css("display", "block");
	};
};
