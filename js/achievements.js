...
Achievement.init = function() {
	...
	for (var i = 0; i < totalMoneyAch.length; i++) {
		...
		$("#ach-totalmoney").append('<li id="ach-totalmoney-' + (i+1) + '" class="list-group-item"><b>' + cnItem(t.name) + '</b> : ' + cnItem(t.text));
	};
	experienceAchOwned = [];
	for (var i = 0; i < experienceAch.length; i++) {
		var e = experienceAch[i];
		experienceAchOwned.push(false);
		$("#ach-experience").append('<li id="ach-experience-' + (i+1) + '" class="list-group-item"><b>' + cnItem(e.name) + '</b> : ' + cnItem(e.text));
	};
	shootAchOwned = [];
	for (var i = 0; i < shootAch.length; i++) {
		var s = shootAch[i];
		shootAchOwned.push(false);
		$("#ach-shoot").append('<li id="ach-shoot-' + (i+1) + '" class="list-group-item"><b>' + cnItem(s.name) + '</b> : ' + cnItem(s.text));
	};
	reloadAchOwned = [];
	for (var i = 0; i < reloadAch.length; i++) {
		var r = reloadAch[i];
		reloadAchOwned.push(false);
		$("#ach-reload").append('<li id="ach-reload-' + (i+1) + '" class="list-group-item"><b>' + cnItem(r.name) + '</b> : ' + cnItem(r.text));
	};
};
