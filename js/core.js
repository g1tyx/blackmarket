...
Init.update = function() {
    if (init == true) {
        ...
        $("#action-reload").html(shoot[0] + "/" + shoot[2] + " 弹药<br>" + fix((shoot[4] / 1000), "time") + " 秒/装弹");
        
        $("#stats-money").html("钱 : <b>$" + fix(money[0], "money") + "</b><br>总金钱 : <b>$" + fix(money[1], "money") + "</b><br>每秒金钱 : <b>$" + fix(moneyPerSec[0] + moneyPerSec[1] + moneyPerSec[2], "money") + "/秒</b>");
        $("#stats-ammo").html("弹夹 : <b>" + shoot[0] + "/" + shoot[2] + "</b><br>总射击数 : <b>" + fix(shoot[5], "money") + "</b><br>总装弹数 : <b>" + fix(shoot[6], "money") + '</b><br>射击所得 <b>' + shootPercent + '% 你的总金钱/秒</b>');
        $("#stats-weed").html("大麻库存 : <b>" + fix(drugStock[0], "drug") + "克</b> (" + fix(drugPerSec[0], "drug") + "克/秒)<br>大麻出售 : <b>" + fix(drugNetPerSec[0], "drug") + "克/秒</b><br>大麻价格 : <b>" + fix(getDrugPrice(0), "drug") + "$/克</b><br>大麻加成 : <b>x" + fix(drugMultiplier[0], "multiplier") + '</b>');
        $("#stats-meth").html("冰毒库存 : <b>" + fix(drugStock[1], "drug") + "克</b> (" + fix(drugPerSec[1], "drug") + "克/秒)<br>冰毒出售 : <b>" + fix(drugNetPerSec[1], "drug") + "克/秒</b><br>冰毒价格 : <b>" + fix(getDrugPrice(1), "drug") + "$/克</b><br>冰毒加成 : <b>x" + fix(drugMultiplier[1], "multiplier") + '</b>');
        $("#stats-cocaine").html("可卡因库存 : <b>" + fix(drugStock[2], "drug") + "克</b> (" + fix(drugPerSec[2], "drug") + "克/秒)<br>可卡因出售 : <b>" + fix(drugNetPerSec[2], "drug") + "克/秒</b><br>可卡因价格 : <b>" + fix(getDrugPrice(2), "drug") + "$/克</b><br>可卡因加成 : <b>x" + fix(drugMultiplier[2], "multiplier") + '</b>');
        ...
        $("#stats-experience").html("经验 : <b>" + fix(prestige[0], "prestige") + "</b><br>重置经验 : <b>" + fix(prestige[1], "prestige") + "</b><br>已消费经验 : <b>" + fix(experienceSpent, "prestige") + "</b>");
        $("#stats-prestige").html("转生称号 : <b>" + cnItem(prestige[3]) + "</b><br>声望加成 : <b>x" + fix(prestige[2], "multiplier") + "</b>");
        
        $("#options-version").html("当前版本 : " + version);
        $("#options-currentFps").html(inputValue + " 帧/秒. ");
