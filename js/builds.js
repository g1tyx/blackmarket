...
Build.init = function() {
	...
	for (var i = 0; i < weedBuilds.length; i++) {
		...
		$("#builds-weed").append('<li id="builds-weed-' + (i+1) + '" class="list-group-item cur-p"><b>' + cnItem(w.name) + '</b> 成本 : $' + fix(getBuildPrice(i, 0), "money") + '<br>生产 ' + fix(getDrugProduction(i, 0), "drug") + "克/秒 <b>大麻</b>");
		$("#builds-weed-" + (i+1)).attr("onclick", 'Build.buy(' + i + ', 0);');
		$("#builds-weed-" + (i+1)).append('<span id="builds-weed-owned-' + (i+1) + '" class="badge">' + weedBuildsOwned[i] + ' 已拥有</span>');
	};
	methBuildsOwned = [];
	for (var i = 0; i < methBuilds.length; i++) {
		var m = methBuilds[i];
		methBuildsOwned.push(0);
		$("#builds-meth").append('<li id="builds-meth-' + (i+1) + '" class="list-group-item cur-p"><b>' + cnItem(m.name) + '</b> 成本 : $' + fix(getBuildPrice(i, 1), "money") + '<br>生产 ' + fix(getDrugProduction(i, 1), "drug") + "克/秒 <b>冰毒</b>");
		$("#builds-meth-" + (i+1)).attr("onclick", 'Build.buy(' + i + ', 1);');
		$("#builds-meth-" + (i+1)).append('<span id="builds-meth-owned-' + (i+1) + '" class="badge">' + methBuildsOwned[i] + ' 已拥有</span>');
	};
	cocaineBuildsOwned = [];
	for (var i = 0; i < cocaineBuilds.length; i++) {
		var c = cocaineBuilds[i];
		cocaineBuildsOwned.push(0);
		$("#builds-cocaine").append('<li id="builds-cocaine-' + (i+1) + '" class="list-group-item cur-p"><b>' + cnItem(c.name) + '</b> 成本 : $' + fix(getBuildPrice(i, 2), "money") + '<br>生产 ' + fix(getDrugProduction(i, 2), "drug") + "克/秒 <b>可卡因</b>");
		$("#builds-cocaine-" + (i+1)).attr("onclick", 'Build.buy(' + i + ', 2);');
		$("#builds-cocaine-" + (i+1)).append('<span id="builds-cocaine-owned-' + (i+1) + '" class="badge">' + cocaineBuildsOwned[i] + ' 已拥有</span>');
	};
};
Build.check = function() {
	...
	for (var i = 0; i < weedBuilds.length; i++) {
		var w = weedBuilds[i];
		$("#builds-weed-" + (i+1)).html('<b>' + cnItem(w.name) + '</b> 成本 : $' + fix(getBuildPrice(i, 0), "money") + '<br>生产 ' + fix(getDrugProduction(i, 0), "drug") + "克/秒 <b>大麻</b>");
		$("#builds-weed-" + (i+1)).append('<span id="builds-weed-owned-' + (i+1) + '" class="badge">' + weedBuildsOwned[i] + ' 已拥有</span>');
	};
	for (var i = 0; i < methBuilds.length; i++) {
		var m = methBuilds[i];
		$("#builds-meth-" + (i+1)).html('<b>' + cnItem(m.name) + '</b> 成本 : $' + fix(getBuildPrice(i, 1), "money") + '<br>生产 ' + fix(getDrugProduction(i, 1), "drug") + "克/秒 <b>冰毒</b>");
		$("#builds-meth-" + (i+1)).append('<span id="builds-meth-owned-' + (i+1) + '" class="badge">' + methBuildsOwned[i] + ' 已拥有</span>');		
	};
	for (var i = 0; i < cocaineBuilds.length; i++) {
		var c = cocaineBuilds[i];
		$("#builds-cocaine-" + (i+1)).html('<b>' + cnItem(c.name) + '</b> 成本 : $' + fix(getBuildPrice(i, 2), "money") + '<br>生产 ' + fix(getDrugProduction(i, 2), "drug") + "克/秒 <b>可卡因</b>");
		$("#builds-cocaine-" + (i+1)).append('<span id="builds-cocaine-owned-' + (i+1) + '" class="badge">' + cocaineBuildsOwned[i] + ' 已拥有</span>');		
	};
};
