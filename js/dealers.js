...
Dealer.init = function() {
	...
	for (var i = 0; i < weedDealers.length; i++) {
		...
		$("#dealers-weed").append('<li id="dealers-weed-' + (i+1) + '" class="list-group-item cur-p"><b>' + cnItem(w.name) + '</b> 成本 : $' + fix(getDealerPrice(i, 0), "money") + '<br>出售 ' + fix(getDealerSell(i, 0), "drug") + '克/秒 <b>大麻</b>');
	...
	for (var i = 0; i < methDealers.length; i++) {
		...
		$("#dealers-meth").append('<li id="dealers-meth-' + (i+1) + '" class="list-group-item cur-p"><b>' + cnItem(m.name) + '</b> 成本 : $' + fix(getDealerPrice(i, 1), "money") + '<br>出售 ' + fix(getDealerSell(i, 1), "drug") + '克/秒 <b>冰毒</b>');
	...
	for (var i = 0; i < cocaineDealers.length; i++) {
		...
		$("#dealers-cocaine").append('<li id="dealers-cocaine-' + (i+1) + '" class="list-group-item cur-p"><b>' + cnItem(c.name) + '</b> 成本 : $' + fix(getDealerPrice(i, 2), "money") + '<br>出售 ' + fix(getDealerSell(i, 2), "drug") + '克/秒 <b>可卡因</b>');
...
Dealer.check = function() {
	...
	for (var i = 0; i < weedDealers.length; i++) {
		var w = weedDealers[i];
		$("#dealers-weed-" + (i+1)).html('<b>' + cnItem(w.name) + '</b> 成本 : $' + fix(getDealerPrice(i, 0), "money") + '<br>出售 ' + fix(getDealerSell(i, 0), "drug") + '克/秒 <b>大麻</b>');
		$("#dealers-weed-" + (i+1)).append('<span id="dealers-weed-owned-' + (i+1) + '" class="badge">' + weedDealersOwned[i] + ' 拥有</span>');
	};
	for (var i = 0; i < methDealers.length; i++) {
		var m = methDealers[i];
		$("#dealers-meth-" + (i+1)).html('<b>' + cnItem(m.name) + '</b> 成本 : $' + fix(getDealerPrice(i, 1), "money") + '<br>出售 ' + fix(getDealerSell(i, 1), "drug") + '克/秒 <b>冰毒</b>');
		$("#dealers-meth-" + (i+1)).append('<span id="dealers-meth-owned-' + (i+1) + '" class="badge">' + methDealersOwned[i] + ' 拥有</span>');	
	};
	for (var i = 0; i < cocaineDealers.length; i++) {
		var c = cocaineDealers[i];
		$("#dealers-cocaine-" + (i+1)).html('<b>' + cnItem(c.name) + '</b> 成本 : $' + fix(getDealerPrice(i, 2), "money") + '<br>出售 ' + fix(getDealerSell(i, 2), "drug") + '克/秒 <b>可卡因</b>');
		$("#dealers-cocaine-" + (i+1)).append('<span id="dealers-cocaine-owned-' + (i+1) + '" class="badge">' + cocaineDealersOwned[i] + ' 拥有</span>');	
