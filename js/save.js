...
function hardReset() {
    if (confirm("您正在尝试永久性删除您的游戏进度，这意味着您将从头开始重新开始所有内容而得不到任何加成或奖励！ 注意：这不是转生/软复位。")) {
...
function softReset() {
    if (confirm("您正在尝试软重置。 这意味着你将重新开始游戏，但会得到 " + fix(prestige[1], "prestige") + " 经验值。 请先查看表格面板，确认下在转生后是否会获得声望加成。")) {
        window.clearInterval(intSave);
        var temp = money[1];
